...
# ===================== Config =====================
SYS_FILE    = "../results/sys/sys_metrics_fedavg_c_50_e_1.csv"  # <- path direto
BINS, SMOOTH_WINDOW = 256, 7
OUT_FIG_MODEL   = "figures/fit/mix_lognorm_shift_fit.png"
OUT_FIG_COMPARE = "figures/fit/mix_lognorm_shift_fit_and_sim.png"
OUT_TXT         = "params/mix_lognorm_shift_params.txt"

RANDOM_STATE = 42
N_SIM_MIN    = 1000  # minimum simulated samples for comparison
FLOPS_PER_SEC = 1e9  # ajuste conforme seu cenário
MS2FLOPS = FLOPS_PER_SEC/1e3
X_LABEL = "FLOPs"    # agora o ajuste é em FLOPs
...
def fit_mix_lognorm_shift_to_smoothed(v, k_range=(1,2,3), loc_grid=None,
    ...
    vmin = float(np.min(v))
    if loc_grid is None:
        # grid em FLOPs equivalente a [vmin-800ms, vmin-100ms]
    ...
    for loc in loc_grid:
        ...
        for k in k_range:
            ...
            # rec = dict(model=f"mixlognorm(k={k})", loc=loc, sse=sse, bic=bic,w=w, mu=mu, sigma=sigma)
            rec = dict(model=f"Mix Lognorm Fuction", loc=loc, sse=sse, bic=bic,w=w, mu=mu, sigma=sigma)
...
def plot_fit_and_simulation(
    ...
    if clip_to_real_xlim: ax1.set_xlim(xmin, xmax)
    ...
    for ax in (ax1, ax2):
        ax.set_ylim(0, 1); ax.set_yticks(yticks)
