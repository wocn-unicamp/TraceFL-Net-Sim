...
def plot_histogram_smoothed(v, out_path=OUT_FIG_HIST):
    ...
    pdf_line, = ax.step(
        edges, np.r_[dens_sm, dens_sm[-1]],
        where="post", lw=1.2, ls="-.", color="black", alpha=0.7,
    ...
    # contorno blanco en la línea ECDF
    ...
    _style_yaxis(ax2)
    # coord x del eje gemelo igual que el principal
...
def plot_mixture_hist_with_ecdf(x, comp, bins=HIST_BINS, mode="stacked", show_points=False, out_path=OUT_FIG_MIXED):
    ...
    # ECDF simulada en eje derecho (mismos ticks/formatter en X)
    ax2 = ax1.twinx()
