...
def trunc_rand(flops, upper, on=True, rng=None):
    """
    Si on=True y upper no es None, cualquier valor flops > upper
    se reemplaza por una uniforme en [0.8*upper, upper] (con eps para evitar igualdades exactas).
    """
    ...
    if np.ndim(u) == 0:
        lo, hi = 0.8*float(u)*(1+eps), float(u)
        flops[m] = rng.uniform(lo, hi, m.sum())
    else:
        lo, hi = 0.8*u[m]*(1+eps), u[m]
...
def composite_cdf_loss(x_s, y_s, w, mu, sigma, loc,
    ...
    # Penalización anti-spike: grande si hay sigmas muy pequeñas
...
def plot_ecdf_validation(v, best, n_sim=N_SIM_MIN, out_path=OUT_FIG_ECDF):
    ...
    # Simulación y modelo consistentes con truncación
    if USE_TRUNCATED_FOR_VALIDATION and "upper_trunc" in best and np.isfinite(best["upper_trunc"]):
        ...
        xs, ys = ecdf_xy(sim)
        xfit = np.linspace(*AX_CFG["xlim"], 1500)
    ...
    # Marca vertical del truncamiento
    if USE_TRUNCATED_FOR_VALIDATION and "upper_trunc" in best and np.isfinite(best["upper_trunc"]):
        u_star = best["upper_trunc"]
        ax.axvline(u_star, ls=":", lw=1.2, color="0.25", alpha=0.9)
        ax.text(u_star, 0.04, TEXTS["common"]["upper_mark_fmt"].format(Fu=best.get("Fu", 0.0)),
                rotation=90, va="bottom", ha="right", fontsize=12, color="0.25")
...
def plot_mixture_hist_with_ecdf(x, comp, bins=HIST_BINS, mode="stacked", show_points=False, out_path=OUT_FIG_MIXED, u_star=None):
    ensure_dir(os.path.dirname(out_path) or "figures")
    colors = ["C0","C1","C2"]
    labels = [TEXTS["mixed"]["labels"]["c1"], TEXTS["mixed"]["labels"]["c2"], TEXTS["mixed"]["labels"]["c3"]]
    fig, ax1 = _figure()
    edges = np.histogram_bin_edges(x, bins=bins)
    widths = np.diff(edges); B = len(widths); lefts = edges[:-1]
    K = int(np.max(comp)) + 1 if comp.size > 0 else 0
    n = x.size; dens = []
    for k in range(K):
        cnt, _ = np.histogram(x[comp == k], bins=edges); dens.append(cnt / (n * widths))
    dens = np.vstack(dens) if dens else np.zeros((0, B))
    ...
    # Sombreado de región truncada (si aplica)
    if u_star is not None and np.isfinite(u_star):
        ax1.axvspan(u_star, AX_CFG["xlim"][1], color="0.9", alpha=0.35, label="Truncation region")
    ...
    if show_points and comp.size > 0:
    ...
    merged_h = h1 + ([h2[0]] if h2 else [])
    merged_l = l1 + ([l2[0]] if l2 else [])
    if merged_h:
        ax1.legend(merged_h, merged_l,
                   loc=LEGEND_CFG["loc"], frameon=LEGEND_CFG["frame"],
                   ncol=LEGEND_CFG["ncol"], fontsize=FONT_CFG["legend"])
    fig.savefig(out_path); plt.close(fig)
...
def plot_ecdf_multi_full(all_curves, out_path):
    ...
    com paleta harmônica e duas legendas (estilo e configuração).
    """
    ensure_dir(os.path.dirname(out_path) or "figures")
    ...
    for i, cur in enumerate(all_curves):
        ...
        ax.step(cur["x_real"], cur["y_real"], where="post", lw=2.0, color=color, alpha=0.9)
        ax.step(cur["x_sim"],  cur["y_sim"],  where="post", lw=1.6, ls="--", color=color, alpha=0.9)
        ax.plot(cur["x_model"], cur["y_model"], lw=1.8, ls=":", color=color, alpha=0.9)
    ...
    ax.set_ylabel("Cumulative Probability", fontsize=15, labelpad=6)
    ax.tick_params(axis="both", labelsize=13, length=5, width=1)
    ax.set_ylim(0, 1)
    ax.grid(True, alpha=0.3)
    _style_xaxis(ax)

    # Legenda por tipo de curva
    style_handles = [
    ...
    leg_style = ax.legend(
        handles=style_handles, title="Curve Type", title_fontsize=13, fontsize=12,
        loc="lower right", bbox_to_anchor=(0.98, 0.5), frameon=True, fancybox=True, edgecolor="0.6", ncol=1
    )
    ax.add_artist(leg_style)

    # Legenda por configuração
    leg_colors = ax.legend(
        handles=color_handles, title="Configuration", title_fontsize=13, fontsize=12,
        loc="lower right", bbox_to_anchor=(0.98, 0.0), frameon=True, fancybox=True, edgecolor="0.6"
    )
...
def main():
    ...
    for base in file_bases:
        ...
        # === Simulación para curvas ALL: usar truncada si corresponde ===
        if USE_TRUNCATED_FOR_VALIDATION and np.isfinite(best_used.get("upper_trunc", np.nan)):
            x_sim = sample_shifted_lognorm_truncated(
                N_SIM_MIN, best_used["loc"], best_used["w"], best_used["mu"], best_used["sigma"],
                u_star=best_used["upper_trunc"], random_state=RANDOM_STATE
            )
        else:
            x_sim = sample_shifted_lognorm_mixture(
                N_SIM_MIN, best_used["loc"], best_used["w"], best_used["mu"], best_used["sigma"],
                random_state=RANDOM_STATE
            )
        x_sim.sort(); y_sim = np.arange(1, len(x_sim)+1)/len(x_sim)

        # === Modelo para curvas ALL: usar CDF truncada si corresponde ===
        x_model = np.linspace(*AX_CFG["xlim"], 1500)
        if USE_TRUNCATED_FOR_VALIDATION and np.isfinite(best_used.get("upper_trunc", np.nan)):
            y_model = cdf_truncated(
                x_model, best_used["w"], best_used["mu"], best_used["sigma"],
                best_used["loc"], best_used["upper_trunc"]
            )
        else:
            y_model = mixture_cdf_lognorm_shift(
                x_model, best_used["w"], best_used["mu"], best_used["sigma"], best_used["loc"]
            )
        ...
        # Etiquetado de componentes simulado solo para color en hist mixto
        ...
        # Pasar u_star para sombreado de región truncada
        plot_mixture_hist_with_ecdf(x_sim, comp_sim, bins=HIST_BINS, mode="stacked",
                                    show_points=False, out_path=out_mix,
                                    u_star=best_used.get("upper_trunc", None))

        # Guardar parámetros + métricas (KS calculado con CDF truncada si aplica)
        ...
        # Alimentar figura combinada con curvas coherentes (truncadas si aplica)
        all_curves.append({
            "label": tag,
