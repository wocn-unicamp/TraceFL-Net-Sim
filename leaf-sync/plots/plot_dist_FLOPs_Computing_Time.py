...
FONT_TICK    = 14
FONT_LEGEND  = 14
FONT_ANNOT   = 12   # (cuadro de hiperparámetros)
...
def sample_capacity_clients(dist: str, C: int, n: int, seed: int | None = None) -> np.ndarray:
    ...
    elif dist == "lognormal":  # <-- NUEVO
        # np.random.lognormal usa mu y sigma del LOG (normal subyacente)
        caps = rng.lognormal(mean=CAP_LOGN_MU, sigma=CAP_LOGN_SIGMA, size=(n, C))
    elif dist == "weibull":
        # CDF: 1 - exp(-(x/λ)^k). rng.weibull usa 'a=k' y escala 1 → escalamos por λ
    ...
    if CLIP_MIN is not None or CLIP_MAX is not None:
        np.clip(caps, CLIP_MIN, CLIP_MAX, out=caps)
    return caps
...
def plot_load_bar(bin_left_edges: np.ndarray,
                  ...
                  bin_size_s: float = 1.0,
                  title_prefix: str = "Mean offered load per second",
    ...
    # ticks 1..N
    n_bins = len(mean_mbps)
    tick_pos = x
    tick_labels = [str(i) for i in range(1, n_bins + 1)]
    try:
