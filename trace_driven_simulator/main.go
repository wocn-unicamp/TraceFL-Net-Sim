...
func main() {
	...
	backgroundTrafficType := flag.String("bg-model", "POISSON", "Probalistic model for background traffic generation. Options: POISSON, PARETO, CBR, MULTI")
	...
	if *backgroundTrafficType != "POISSON" && *backgroundTrafficType != "PARETO" && *backgroundTrafficType != "CBR" && *backgroundTrafficType != "MULTI" {
		log.Panic("Invalid background traffic model. Please choose from: POISSON, PARETO, CBR, MULTI")
	}
